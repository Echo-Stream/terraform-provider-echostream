...
func (d *ProcessorFunctionDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if echoResp.GetFunction != nil {
		...
		case *api.ReadFunctionGetFunctionProcessorFunction:
			...
			config.ReturnMessageType = types.String{Value: function.ReturnMessageType.Name}
		default:
			resp.Diagnostics.AddError(
				"Incorrect Function type",
				fmt.Sprintf("expected ProcessorFunction, got %v", function.GetTypename()),
			)
			return
		}
	} else {
		resp.Diagnostics.AddWarning("Unable to find function", fmt.Sprintf("'%s' function does not exist", config.Name.Value))
		config = processorFunctionModel{}
	}
